...
class SimpleWeatherScraper:
    ...
    def print_city_weather(self, city: str, days: int = 7):
        """打印指定城市的天气信息"""
        weather_data = self.get_weather_data(city, days)
        
        print(f"\n{city} 未来 {days} 天天气:")
        print("-" * 50)
        for weather_info in weather_data:
            print(f"{weather_info.date}: {weather_info.weather} - {weather_info.temperature} "
                  f"(湿度: {weather_info.humidity}, 风速: {weather_info.wind_speed})")

def main():
    """主函数 - 使用示例"""
    print("🌤️  简化版天气爬取和分析系统")
    print("=" * 50)
    ...
    if result:
        ...
        print(f"🌤️  天气: {weather_info.weather} ({weather_info.description})")
        print(f"🌡️  温度: {weather_info.temperature}")
        print(f"💧 湿度: {weather_info.humidity}")
        print(f"💨 风速: {weather_info.wind_speed}")
    else:
        print("❌ 未找到合适的晴天城市")
    ...
    # 示例3: 显示城市天气详情
    print("\n示例3: 显示北京未来3天天气详情")
    scraper.print_city_weather('北京', 3)
